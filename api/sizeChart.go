...
func UpdateSizeChartBySpu(c *gin.Context) {
	...
	productSrv.UpdateSizeChartBySpu(data.SizeChart, data.SpuList)
	c.JSON(200, "success")
}

func UpdateSizeByProductId(c *gin.Context) {
	type Data struct {
		TableName  string `form:"table_name" json:"table_name"  binding:"required"`
	...
	productSrv := services.Product{TableName: data.TableName}
	productSrv.UpdateSizeChartBySProductIds(data.Size, data.ProductIds)
	c.JSON(200, "success")
}
